...
public class HomeDaoYaml implements HomeDao {
	...
	public void insert(Home home) {
		...
		homesDataConfig.set(path + ".ownerName", home.getOwnerName());
	...
	public Home findById(int id) {
		...
		String ownerName = homesDataConfig.getString("homes." + id + ".ownerName");
		...
		return new Home(id, homeName, ownerName, location, icon);
	}

	@Override
	public List<Home> findPlayerHomes(String playerName) {
		...
		for (String id : homesDataConfig.getConfigurationSection("homes").getKeys(false)) {
			Home home = this.findById(Integer.parseInt(id));
